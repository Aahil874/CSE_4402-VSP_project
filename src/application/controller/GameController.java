...
import application.model.HitResult;
import application.model.Mascot;
import application.model.MascotAnimationState;
import application.model.MatchState;
import application.model.PitchType;
import application.model.Runner;
import application.model.ScoreboardState;
import application.model.TeamLineup;
import application.model.WorldPoint;
...
public final class GameController {
    ...
    private void endPitchSequence() {
        // 1. Reset ball & pitch tracking flags
        pitchActive = false;
        pitchTravel = 0.0;
        ballX = BALL_START_X;
        ballY = BALL_RELEASE_Y;

        // 2. Safely advance state machine back to ready
        if (playStateController != null) {
            if (playStateController.is(BaseballPlayStateController.State.BATTER_SWINGING) ||
                playStateController.is(BaseballPlayStateController.State.PITCH_IN_FLIGHT)) {
        ...
        // 3. Refresh HUD and UI controls for the next pitch
        updateHud();
    ...
    private void initialize() {
        ...
        // Pass frame dimensions (320x384 cell width/height) to the swing controller
        // Note: If you draw directly on gameCanvas, batSwingController will track 
        // the frame index internally to draw the viewport frame onto the canvas.
        ...
        gameLoop = new AnimationTimer() {
            ...
            public void handle(long now) {
                ...
                if (!paused) {
                    // Update swing animation state & sprite sheet frame step
    ...
    /**
     * Call this when the player or rival triggers a swing action.
     */
    private void triggerBatSwing() {
        if (batSwingController != null) {
            batSwingController.startSwing();
        }
    }

    private void executeRivalSwing() {
        // 1. Trigger visual sprite swing animation
        if (batSwingController != null) {
            batSwingController.startSwing();
        }

        // 2. Transition state machine safely through BATTER_SWINGING
        if (playStateController != null) {
            if (playStateController.is(BaseballPlayStateController.State.PITCH_IN_FLIGHT)) {
                playStateController.transition(BaseballPlayStateController.State.BATTER_SWINGING);
            }
            
            // Resolve the pitch state back to play transition flow
            if (playStateController.is(BaseballPlayStateController.State.BATTER_SWINGING)) {
                playStateController.transition(BaseballPlayStateController.State.PLAY_RESOLVED);
            }
        }

        // 3. Evaluate timing and contact via BattingController/PitchSequenceController
        double timingDelta = Math.abs(pitchTravel - 0.85);
        
        // (Optional) Delegate gameplay calculations to your active batting/pitch controllers
        if (battingController != null) {
            // e.g., battingController.evaluateSwing(timingDelta);
        }
    ...
    private void drawGame() {
        ...
        // 2. Realistic Stadium Backdrop (Replaces drawBackdrop & drawFieldGlow)
        if (stadiumBackground != null) {
            graphics.drawImage(stadiumBackground, 0, 0, width, height);
        } else {
            drawBackdrop(graphics, width, height);
            drawFieldGlow(graphics, width, height);
        }

        // 3. Field & Fielder Layering
        ...
        // 4. Player & Pitcher Layering driven by BatSwingController
        if (phase == Phase.PITCHING) {
            // Player Pitching (Mound)
            drawMascotPlayer(graphics, 535, 185,
                    displayedPlayerMascot(), false, 0.46);
            
            // Rival Batting (Plate) - Linked to swing controller phase
            ...
            // Rival Pitching (Mound)
            drawRivalPlayer(graphics, 535, 185, false);
            
            // Player Batting (Plate) - Driven by active swing frame
        ...
        // 5. Aiming Reticle / Strike Zone
        if (GameSettings.isStrikeZoneVisible()) {
            drawAimReticle(graphics);
        }

        // 6. Realistic Ball Flight Trajectory & Ground Shadow
        if (pitchActive) {
            drawRealisticPitch(graphics);
        ...
        // 7. Hit Effects, Particles, & Floating Text
        if (contactFlash > 0) {
    ...
    /**
     * Helper to render pitch shadow on the field grass for added depth.
     */
    private void drawRealisticPitch(GraphicsContext graphics) {
        // Dynamic pitch shadow mapping ball position to ground
        double shadowY = BALL_RELEASE_Y + (pitchTravel * (PLATE_Y - BALL_RELEASE_Y));
        double shadowRadius = 4 + (pitchTravel * 6);

        graphics.setFill(Color.color(0, 0, 0, 0.35));
        graphics.fillOval(ballX - (shadowRadius / 2), shadowY, shadowRadius * 2, shadowRadius * 0.6);
    }

    private void drawBackdrop(GraphicsContext graphics, double width, double height) {
        // 1. Primary Path: Render high-res perspective stadium asset
        if (stadiumBackground != null && !stadiumBackground.isError()) {
            drawCoverImage(graphics, stadiumBackground, width, height);
            
            // Subtle ambient lighting tint for depth and contrast
            graphics.setFill(Color.rgb(8, 14, 42, 0.18));
            graphics.fillRect(0, 0, width, height);
            return;
        }

        // 2. Clean Modern Fallback (Replaces pixelated grid crowd & blocky grass)
        // Deep stadium twilight sky/backdrop
        graphics.setFill(Color.web("#0e1a38"));
        graphics.fillRect(0, 0, width, height * 0.35);

        // Dark outfield wall boundary
        graphics.setFill(Color.web("#152244"));
        graphics.fillRect(0, height * 0.30, width, height * 0.05);

        // Ground/Grass field
        graphics.setFill(Color.web("#1e421e"));
    ...
    private void drawBallAndTrail(GraphicsContext graphics) {
        ...
        for (int step = 4; step >= 1; step--) {
            ...
            // Scale trail size dynamically from mound to plate (Perspective growth)
            double trailSize = (6 + (5 - step) * 2) * (0.6 + (trailProgress * 0.6));
            
            graphics.setFill(Color.rgb(255, 240, 170, 0.12 * (5 - step)));
            graphics.fillOval(trailX - trailSize / 2, trailY - trailSize / 2, trailSize, trailSize);
        }

        // Ball perspective scale: Starts smaller at release point (mound), expands at plate
        double ballScale = 0.55 + (pitchTravel * 0.65); // 55% scale at mound -> 120% scale at plate
        double currentRadius = 8 * ballScale;

        // Draw main ball body
        graphics.setFill(Color.WHITE);
        graphics.fillOval(ballX - currentRadius, ballY - currentRadius, currentRadius * 2, currentRadius * 2);

        // Ball seams
        graphics.setStroke(Color.web("#ff3b30"));
        graphics.setLineWidth(1.5 * ballScale);
        graphics.strokeArc(ballX - currentRadius * 0.75, ballY - currentRadius * 0.75,
                currentRadius * 1.5, currentRadius * 1.5, 25, 100, javafx.scene.shape.ArcType.OPEN);
        graphics.strokeArc(ballX - currentRadius * 0.75, ballY - currentRadius * 0.75,
                currentRadius * 1.5, currentRadius * 1.5, 205, 100, javafx.scene.shape.ArcType.OPEN);
    }
...
// Determine horizontal frame index (0..5) based on swing phase or pitching state
    			int frameColumn = 0;
    			if (batting) {
// Map Phase enum directly to Sprite Sheet column
    				switch (batSwingController.phase()) {
    				case LOAD:           frameColumn = 1; break;
    				case SWING:          frameColumn = 2; break;
    				case CONTACT:        frameColumn = 3; break;
    				case FOLLOW_THROUGH: frameColumn = 4; break;
    				case RECOVERY:       frameColumn = 5; break;
    				case READY:
    				default:             frameColumn = 0; break;
    				}
    			} else {
    				frameColumn = pitchAnimation > 0 ? 2 : 0;
}

    // Shadow Layering (Matches perspective ground)
    			graphics.save();
	graphics.setFill(Color.rgb(0, 0, 0, 0.35));
	graphics.fillOval(x + displayWidth * 0.15, y + displayHeight * 0.88,
			displayWidth * 0.70, displayHeight * 0.12);
	graphics.restore();

	// Sprite Sheet Viewport Drawing
	graphics.save();
	double sourceX = frameColumn * SPRITE_CELL_WIDTH;
	double sourceY = mascot.ordinal() * SPRITE_CELL_HEIGHT;

	// Draw active sprite crop to canvas
	graphics.drawImage(
			gameplaySprites,
			sourceX, sourceY, SPRITE_CELL_WIDTH, SPRITE_CELL_HEIGHT,
			x, y, displayWidth, displayHeight);

	if (batting && batSwingController.collisionEnabled()) {
