...
public final class GameOverController {
    ...
    private void initialize() {
        ...
        if (DatabaseConnection.isOfflineMode() || !DatabaseConnection.isConfigured()) {
            ...
            saveButton.setText("OFFLINE");
            saveStatusLabel.setText("GAME COMPLETE - CONNECT MYSQL LATER TO SAVE STATS");
        }
    }
    ...
    private void handleSaveScore() {
        if (DatabaseConnection.isOfflineMode() || !DatabaseConnection.isConfigured()) {
            saveStatusLabel.setText("OFFLINE MODE - GAMEPLAY RESULT KEPT FOR THIS SESSION");
            return;
        }
